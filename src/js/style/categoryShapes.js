 ...
 *
 */
import {symbol, symbolCross, symbolDiamond, symbolSquare, symbolStar, symbolTriangle, symbolWye} from "d3";

const ol = window.ol;
const {Polygon, Circle} = ol.geom;
const {toContext} = ol.render;
const {Style, Fill, Stroke} = ol.style;

const shapes = [null, symbolCross, symbolDiamond, symbolSquare, symbolStar, symbolTriangle, symbolWye];
...
function createRenderer(categoryMap) {
    return (location, {context, feature}) => {
        ...
        if (points.length) {
            const sizedPoints = points.map(p => [p[0] * scale + location[0], p[1] * scale + location[1]]);
